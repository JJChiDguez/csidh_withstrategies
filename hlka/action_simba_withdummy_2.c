...
void action_evaluation(proj C, const uint8_t key[], const proj A)
{
  ...
  // SIMBA parameters
  // Batches
  uint8_t batches[NUMBER_OF_BATCHES][74]; //Set manually to largest possible batch size
  ...
  while (isog_counter < NUMBER_OF_ISOGENIES)
  {
    ...
    {
      //printf("%3d",complement_of_each_batch[m][i]); 
	...
	point_copy(G[0], current_T[0]);	// order-l point determined by T_{-}
      ...
      fp_cswap(G[0][0], G[1][0], (ec & 1));		// constant-time swap: T_{+} or T_{-}
      fp_cswap(G[0][1], G[1][1], (ec & 1));		// constant-time swap: T_{+} or T_{-}
      fp_cswap(G[2][0], G[3][0], (ec & 1));		// constant-time swap: T_{+} or T_{-}
      fp_cswap(G[2][1], G[3][1], (ec & 1));		// constant-time swap: T_{+} or T_{-}

      fp_cswap(current_T[0][0], current_T[1][0], (ec & 1));		// constant-time swap: T_{+} or T_{-}
      fp_cswap(current_T[0][1], current_T[1][1], (ec & 1));		// constant-time swap: T_{+} or T_{-}
	...
	fp_cswap(G[0][0], G[2][0], bc);		// constant-time swap: dummy or not dummy
	fp_cswap(G[0][1], G[2][1], bc);		// constant-time swap: dummy or not dummy
	...
	if ( isequal(batches[m][i], last_isogeny[m]) == 0)	// constant-time ask: just for avoiding the last isogeny evaluation
	{
	  ...
	  fp_cswap(current_T[0][0], current_T[2][0], bc ^ 1);		// constant-time swap: dummy or not dummy
	  fp_cswap(current_T[0][1], current_T[2][1], bc ^ 1);		// constant-time swap: dummy or not dummy
	  fp_cswap(current_T[1][0], current_T[3][0], bc ^ 1);		// constant-time swap: dummy or not dummy
	  fp_cswap(current_T[1][1], current_T[3][1], bc ^ 1);		// constant-time swap: dummy or not dummy

	};

	fp_cswap(current_A[0][0], current_A[1][0], bc ^ 1);	// constant-time swap: dummy or not dummy
	fp_cswap(current_A[0][1], current_A[1][1], bc ^ 1);	// constant-time swap: dummy or not dummy
	...
	yMUL(current_T[1], current_T[1], current_A[0], batches[m][i]);
      ...
      fp_cswap(current_T[0][0], current_T[1][0], (ec & 1));		// constant-time swap: dummy or not dummy
      fp_cswap(current_T[0][1], current_T[1][1], (ec & 1));		// constant-time swap: dummy or not dummy

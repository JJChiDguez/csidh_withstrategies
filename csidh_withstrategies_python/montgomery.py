...
try:
    ...
    f = open('./sdacs/' + sys.argv[1],'w')
    for i in range(0, n, 1):

        f.writelines(' '.join([ str(tmp) for tmp in SDACS[i]]) + '\n')
    f.close()

cMUL  = lambda l: np.array( [ 4.0 * (SDACS_LENGTH[L.index(l)] + 2), 2.0 * (SDACS_LENGTH[L.index(l)] + 2), 6.0 * (SDACS_LENGTH[L.index(l)] + 2) - 2.0] )
...
def xDBL(P, A):
    ...
    FIELD_OPERATIONS_PERFORMED += np.array([4.0, 2.0, 4.0])
...
def xADD(P, Q, PQ):
    ...
    global FIELD_OPERATIONS_PERFORMED
    FIELD_OPERATIONS_PERFORMED += np.array([4.0, 2.0, 6.0])
...
def elligator(A):
    ...
    global FIELD_OPERATIONS_PERFORMED
    FIELD_OPERATIONS_PERFORMED += np.array([7.0, 3.0, 10.0])
    ...
    return [Tp_X, C_times_u_squared_minus_one], [Tm_X, C_times_u_squared_minus_one]
...
def CrissCross(alpha, beta, gamma, delta):
    ...
    global FIELD_OPERATIONS_PERFORMED
    FIELD_OPERATIONS_PERFORMED += np.array([2.0, 0.0, 2.0])
